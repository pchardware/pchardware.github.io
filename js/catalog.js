...
function updateView() {
    ...
    updateFilters("Características", 3, keywords.sort(), true, "feature");
}

function getKeywords(products) {
    var keywords = [];
    var notCovereds = products.slice(); // Hacer una copia de la colección de productos
    while (notCovereds.length > 0) {
        // Contar la cantidad de veces que aparece cada palabra clave en los productos no cubiertos
        var qty = {};
        notCovereds.forEach(function (product) {           
            product.forEach(function (feature) {
                if (qty[feature]) {
                    qty[feature]++;
                } else {
                    qty[feature] = 1;
        ...
        // Seleccionar la palabra clave que cubre la mayor cantidad de productos no cubiertos
        var keywordSelected = null;
        var covereds = {};
        Object.keys(qty).forEach(function (keyword) {
            var coveredQty = 0;
            notCovereds.forEach(function (product) {
            ...
            covereds[keyword] = coveredQty;
            if (!keywordSelected || covereds[keyword] > covereds[keywordSelected]) {
                keywordSelected = keyword;
            }
        });

        // Agregar la palabra clave seleccionada al arreglo de palabras clave y eliminar los productos cubiertos
        keywords.push(keywordSelected);
        notCovereds = notCovereds.filter(function (product) {
            return !product.includes(keywordSelected);
        });
    }
    
    // Eliminar las palabras clave redundantes
    keywords = keywords.filter(function (keyword, index) {
        for (var i = 0; i < index; i++) {
            if (covereds[keywords[i]] && covereds[keyword] === covereds[keywords[i]]) {
                return false;
            }
        }
        return true;
